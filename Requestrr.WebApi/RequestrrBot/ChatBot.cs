...
namespace Requestrr.WebApi.RequestrrBot
{
    public class ChatBot
    {
        ...
        private OverseerrWebhookService _overseerrWebhookService;
        ...
        public ChatBot(IServiceProvider serviceProvider, ILogger<ChatBot> logger, DiscordSettingsProvider discordSettingsProvider)
        {
            ...
            _overseerrWebhookService = serviceProvider.Get<OverseerrWebhookService>();
        }
        ...
        private async Task Connected(DiscordClient client, ReadyEventArgs args)
        {
            ...
            // Set the Discord client for the webhook service
            if (_overseerrWebhookService != null)
            {
                _overseerrWebhookService.SetDiscordClient(client);
            }
        }
        ...
        private async Task DiscordComponentInteractionCreatedHandler(DiscordClient client, ComponentInteractionCreateEventArgs e)
        {
            try
            {
                // Handle Overseerr webhook moderation buttons
                if (e.Id.StartsWith("overseerr_"))
                {
                    await _overseerrWebhookService.HandleButtonInteractionAsync(client, e);
                    return;
                }
